...
{p2colset 5 20 20 2}{...}
{p2col :{hi:clan} {hline 2}}Cluster-level analysis of data from a cluster randomised trials{p_end}
{p2colreset}{...}
...
penalising by: one to account for each treatment; one fewer than the
...
{phang}
{opt eff:ect(effect)} specifies which measure of effect you wish to calculate. If rr or 
rater are specified, the confidence interval will be calculated on the log scale
and the estimate will be the ratio of geometric means of the cluster summaries. If any
cluster has zero events, 0.5 will be added to all cluster totals to allow
logarithms to be taken.

{phang}
{opth str:ata(varname)} is the variable which identifies the stratification used in the trial randomisation. Only one stratification factor is permitted. It must be a numeric variable.
...
when either rate differences or ratios are to be calculated.
